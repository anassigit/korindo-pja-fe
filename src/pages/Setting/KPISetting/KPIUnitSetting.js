...
import { deleteKPIUnit, getKPIUnitList, resetMessage } from "store/actions"
...
import AddKPIUnit from "./AddKPIUnit"
import EditKPIUnit from "./EditKPIUnit"

const KPIUnitSetting = (props) => {
    ...
    const appListData = useSelector((state) => {
        return state.settingReducer.respGetKPIUnitList
    })
    ...
    const toggleDeleteModal = (data) => {
        setModal(!modal)
        if (data?.unitId) {
            setSelectedId(data.unitId)
        }
    }

    useEffect(() => {
        if (isYes) {
            dispatch(deleteKPIUnit({ unitId: selectedId }))
    ...
    useEffect(() => {
        ...
        if (appMessageDelete.status === '1' || appMessageDelete.status === '0') {
            ...
            dispatch(getKPIUnitList(searchTable))
        }
        if (messageToUpdate) {
...
KPIUnitSetting.propTypes = {
    location: PropTypes.object,
    t: PropTypes.any
}

export default withTranslation()(KPIUnitSetting)