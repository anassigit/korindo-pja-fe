...
import { deleteKPICategory, getKPICategoryList, resetMessage } from "store/actions"
...
import EditKPICategory from "./EditKPICategory"

const KPICategorySetting = (props) => {
    ...
    const appListData = useSelector((state) => {
        return state.settingReducer.respGetKPICategoryList
    })
    ...
    useEffect(() => {
        if (isYes) {
            dispatch(deleteKPICategory({ categoryId: selectedId }))
    ...
    useEffect(() => {
        ...
        if (appMessageDelete.status === '1' || appMessageDelete.status === '0') {
            ...
            dispatch(getKPICategoryList(searchTable))
        }
        if (messageToUpdate) {
...
KPICategorySetting.propTypes = {
    location: PropTypes.object,
    t: PropTypes.any
}

export default withTranslation()(KPICategorySetting)