...
import { getSelectFile, deleteFileFolder, resetMessage, getSearch } from "../../store/FileManagement/actions"
...
const DataInquiry = (props) => {
    ...
    const searchFileData = useSelector(state => {
        return state.fileManagementReducer.respGetSearchFile
    })
    const checkDownloadMsg = useSelector(state => {
        return state.fileManagementReducer.respGetDownloadCheck
    })
    const deleteFileMsg = useSelector(state => {
    ...
    useEffect(() => {
        if (selectFileData) {
            setRealFileList(selectFileData?.data?.childList)
            setRealFilePath(selectFileData?.data?.path)
    ...
    useEffect(() => {
        setEnterMonthlyDataSpinner(false)
        if (searchFileData?.data !== null) {
            setRealFileList(searchFileData?.data?.searchList)
        } else if (searchFileData?.data === null) {
            setRealFileList(selectFileData?.data?.childList)
        }
    }, [searchFileData])
    ...
    }, [downloadFlag, tempFileName, tempNum])
    ...
    useEffect(() => {
        if (checkDownloadMsg.status === "0") {
            setDownloadMsg(checkDownloadMsg)
            toggleMsgModal()

        } else if (checkDownloadMsg.status === "1") {
            downloadFolderFile()
        }
        setDownloadContentModal(checkDownloadMsg.message)
        setDownloadMsg("")
    ...
    const handleSearchChange = () => {
        setEnterMonthlyDataSpinner(true)
        dispatch(getSearch({ "search": searchValue }))
    }
